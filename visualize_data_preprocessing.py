...
class DataPreprocessingVisualizer:
    ...
    def analyze_data_segments(self, n_segments=10):
        """
    ...
    def load_real_data(self, segment_index=None, sample_rate=0.1, use_full_data=False):
        """
        读取真实校准数据
        
        Args:
            segment_index: 数据段索引 (None=自动选择, 0=前10%, 1=10%-20%, ...)
            sample_rate: 每段采样率 (0.1 = 10%)
            use_full_data: 是否使用全部数据 (True=读取所有200万行, False=读取10,000行子集)
        """
        # 如果使用全部数据
        if use_full_data:
            print(f"\n正在读取全部数据...")
            chunk_size = None  # 读取所有数据
            skip_rows = None
        else:
            # 如果未指定段索引，自动分析选择
            if segment_index is None:
                segment_index = self.analyze_data_segments()
                if segment_index is None:
    ...
    def plot_steady_state_selection(self):
        ...
        ax1.set_ylabel('RPM', fontsize=14, color=self.colors['primary'])
        ax1_twin.set_ylabel('RPM标准差', fontsize=14, color=self.colors['danger'])
    ...
    def plot_representative_points(self):
        ...
        if rpm_range > 10 and len(steady_df) > n_points:
            ...
            ax2.scatter(steady_df['rpm'], steady_df['P_max'],
                       s=25, alpha=0.5, color=self.colors['success'])
    ...
    def plot_normalization_correlation(self):
        ...
        if 'P_comp_est' in df.columns:
            ...
            valid_mask = df['P_comp'].notna() & df['P_comp_est'].notna()
            if valid_mask.sum() > 0:
                r2 = r2_score(df.loc[valid_mask, 'P_comp'], df.loc[valid_mask, 'P_comp_est'])
...
def main():
    ...
    try:
        # 读取第8段数据（80,000-90,000行，特征最鲜明）
        # use_full_data=True: 读取全部数据 (约300MB内存)
        # use_full_data=False: 读取10,000行子集 (自动选择最佳数据段)
        visualizer.load_real_data(use_full_data=False)
        
        # 按优先级顺序生成所有可视化
        visualizer.plot_steady_state_selection()          # 1. 稳态筛选
        visualizer.plot_representative_points()            # 2. 工况点提取
        visualizer.plot_data_cleaning()                    # 3. 异常值剔除
        visualizer.plot_normalization_correlation()        # 4. 标准化处理
        ...
        # 显示使用的数据段信息
        if visualizer.segment_stats is not None:
            print(f"\n使用的数据段统计:")
            print(visualizer.segment_stats.to_string(index=False))
