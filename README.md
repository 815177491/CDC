...
CDC/
├── main.py                    # 主入口脚本
├── run_gpu_comparison.py      # 🆕 GPU加速RL对比实验脚本
├── run_rl_comparison.py       # RL算法对比脚本
...
│   ├── diagnoser.py           # 残差诊断器 (RandomForest)
│   ├── kan_diagnoser.py       # 🆕 KAN诊断器 (MIT 2024)
│   ├── pinn_diagnoser.py      # 🆕 PINN诊断器 (物理信息神经网络)
│   └── hybrid_diagnoser.py    # 🆕 混合诊断器 (KAN为主+PINN为辅)
│
├── control/                   # 协同控制模块
...
│   ├── control_agent.py       # 控制智能体 (DQN/SAC)
│   ├── rl_algorithms.py       # 基础RL算法 (DQN/PPO/SAC/TD3)
│   ├── advanced_rl_algorithms.py  # 🆕 2024-2025前沿算法
│   ├── multi_algo_control.py  # 多算法控制接口
...
| 算法 | 年份 | 来源 | 类型 | 特点 |
|------|------|------|------|------|
| DQN | 2015 | Nature | Value-based | 基线方法，经验回放+目标网络 |
| SAC | 2018 | ICML | Actor-Critic | 最大熵框架，探索性好 |
| TD-MPC2 | 2024 | ICLR | Model-based | 世界模型+MPC规划 ★推荐 |
| DPMD | 2025 | - | Generative | 扩散策略+镜像下降 |

## 支持的诊断方法

| 方法 | 年份 | 特点 |
|------|------|------|
| RandomForest | 经典 | 集成分类器，稳定性好 |
| KAN | 2024 | MIT最新，可学习激活函数，可解释性强 |
| PINN | 2024 | 物理信息神经网络，嵌入热力学约束 |
| Hybrid | 2025 | KAN为主+PINN为辅，投票机制融合 |
