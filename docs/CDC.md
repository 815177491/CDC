...
_【图1插入位置：故障诊断流程图，包括残差计算、阈值检测、特征匹配和故障分类步骤，建议放置于../results/fig_diagnosis_flowchart.png】_
...
控制模式分为四种：正常模式（NORMAL）下保持当前控制参数不变；容错模式（FAULT*TOLERANT）下启动VIT和燃油的协调调整；降级模式（DEGRADED）下在健康气缸之间重新分配负荷；紧急模式（EMERGENCY）下执行快速降功保护。模式切换的条件由诊断状态和$P*{\text{max}}$测量值共同决定。
...
_【图2插入位置：Pmax越限协同控制原理框图，包括诊断模块、PID控制器、VIT执行器和燃油执行器，建议放置于../results/fig_pmax_control_block.png】_
...
_【图3插入位置：故障响应时序图（fig3*fault_response.png），建议放置于../results/fig3_fault_response.png。该图应包含三个子图：(A)故障注入信号随时间的变化；(B)$P*{\text{max}}$的开环响应、闭环响应和安全限值对比；(C)VIT和燃油的控制调整量。】_
...
_【图4插入位置：性能权衡雷达图（fig4_performance_radar.png），建议放置于../results/fig4_performance_radar.png。该图应展示三种状态的雷达图叠加：正常基准（绿色虚线）、故障发生时（红色实线填充）、协同控制后（蓝色实线填充）。】_
...
_【该图应放置于../results/fig1_calibration_verification.png】_
...
_【该图应放置于../results/fig2_pv_diagram.png】_
...
_【该图应放置于../results/fig5_heat_release.png】_
...
_【图5插入位置：燃烧放热率曲线（fig5_heat_release.png），建议放置于../results/fig5_heat_release.png】_

_【图6插入位置：控诊协同系统总体架构图，建议放置于../results/fig_cdc_architecture.png】_
